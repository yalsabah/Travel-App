...
export function AuthPage() {
  ...
  async function handleAdminLogin() {
    if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
      setError('Admin credentials not set in .env.local')
      return
    }
    setError('')
    setLoading(true)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-950 px-4">
      ...
      <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-700 p-8">
        ...
        <div className="text-center mb-7">
          ...
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Europe Trip Planner</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">May 31 – Jun 13/20, 2026</p>
        </div>
      ...
      <p className="text-xs text-slate-400 mt-3">Your trip data is saved securely to your account.</p>
    </div>
  )
}
